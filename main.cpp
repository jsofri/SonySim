...
vector<const char *> tokens_array;
SymbolTable symbol_table;

int main() {
    VarCommand var_command;

    tokens_array = {"var", "x", "-", ">", "sim", "(", "reference//address/to/simulator/variable/here", ")", "\n",
                    "var", "y", "\n",
                    "x", "=" , "9", "\n",
                    "y", "=", "x", "*", "3", "+", "1", "\n"};
    int n, index = 0;

    n = sizeof(tokens_array) / sizeof(tokens_array[0]);

    while (index <= n) {
        index += var_command.execute(index);
    }
